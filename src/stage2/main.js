...
function addOvertureBuildings() {
  ...
  if (!map.getLayer('overture-building-2d')) {
    map.addLayer({
      ...
      paint: { 'fill-color': ['case', ['boolean', ['get', 'has_parts'], false], '#d8d1c7', '#d2cbc0'], 'fill-opacity': 0.52, 'fill-outline-color': '#b7afa3' }
    }, before);
  }
...
function installOverlays() {
  ...
  setStatus(`地图底座已连接 · Overture ${OVERTURE_RELEASE}`, 'ok');
...
const map = new maplibregl.Map({
  container: 'map', style: OPENFREEMAP_STYLE, center: FOSHAN.center, zoom: 10.7, pitch: 0, bearing: 0,
...
// style.load is the correct startup barrier for registering our own sources/layers.
// map.load can be delayed by slow remote vector tiles, especially in CI or high-latency networks.
map.on('style.load', () => {
...
map.on('load', () => { document.documentElement.dataset.mapLoaded = 'true'; });
